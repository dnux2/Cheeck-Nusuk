...
import { Shield, Users, Eye, EyeOff, LogIn, Lock, User, CreditCard, Hash, AlertCircle } from "lucide-react";
...
export function LoginPage() {
  ...
  const handleSupervisorLogin = async (e: React.FormEvent) => {
    ...
    if (result.ok) {
      ...
      setLoading(false);
      setErrorMsg(result.error || (ar ? "اسم المستخدم أو كلمة المرور غير صحيحة" : "Invalid username or password"));
    }
  };
  ...
  const handlePilgrimLogin = async (e: React.FormEvent) => {
    ...
    if (result.ok) {
      ...
      setLoading(false);
      setErrorMsg(result.error || (ar ? "رقم الجواز أو الرمز السري غير صحيح" : "Invalid passport number or PIN"));
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#0E4D41] via-[#0a3d32] to-[#052a22] flex flex-col items-center justify-center px-4 py-8" dir={isRTL ? "rtl" : "ltr"}>
      ...
      {/* Language toggle */}
      <div className={`absolute top-4 ${isRTL ? "left-4" : "right-4"} flex items-center bg-white/10 backdrop-blur-sm rounded-lg overflow-hidden border border-white/20`}>
      ...
      >
        ...
        <div className="flex border-b border-border">
          <button
            ...
            onClick={() => { setTab("supervisor"); setErrorMsg(""); }}
          ...
          <button
            ...
            onClick={() => { setTab("pilgrim"); setErrorMsg(""); }}
          ...
          <AnimatePresence>
            {errorMsg && (
              <motion.div
                ...
                className={`mb-4 flex items-center gap-2 bg-destructive/10 text-destructive border border-destructive/20 rounded-2xl px-4 py-3 text-sm font-medium ${isRTL ? "flex-row-reverse text-right" : ""}`}
              >
          ...
          <AnimatePresence mode="wait">
            {tab === "supervisor" ? (
              <motion.form
                ...
                className="space-y-5"
              >
                <p className={`text-xs font-semibold text-muted-foreground ${isRTL ? "text-right" : ""}`}>
                  {ar ? "الدخول للوحة تحكم المشرفين" : "Access the supervisor control panel"}
                </p>

                <div className="space-y-1.5">
                  <label className={`text-xs font-bold text-foreground block ${isRTL ? "text-right" : ""}`}>{ar ? "اسم المستخدم" : "Username"}</label>
                ...
                <div className="space-y-1.5">
                  <label className={`text-xs font-bold text-foreground block ${isRTL ? "text-right" : ""}`}>{ar ? "كلمة المرور" : "Password"}</label>
                  <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowPassword(v => !v)} className={`absolute top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors ${isRTL ? "left-3" : "right-3"}`}>
                ...
                >
                  {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><LogIn className="w-4 h-4 flex-shrink-0" />{ar ? "تسجيل الدخول" : "Sign In"}</>}
                </button>
              </motion.form>
            ) : (
              <motion.form
              ...
              >
                <p className={`text-xs font-semibold text-muted-foreground ${isRTL ? "text-right" : ""}`}>
                  {ar ? "ادخل برقم جواز سفرك والرمز السري (آخر 4 أرقام من الهاتف)" : "Sign in with your passport number and PIN (last 4 digits of phone)"}
                </p>

                <div className="space-y-1.5">
                  <label className={`text-xs font-bold text-foreground block ${isRTL ? "text-right" : ""}`}>{ar ? "رقم جواز السفر" : "Passport Number"}</label>
                  <div className="relative">
                ...
                <div className="space-y-1.5">
                  <label className={`text-xs font-bold text-foreground block ${isRTL ? "text-right" : ""}`}>{ar ? "الرمز السري (4 أرقام)" : "4-Digit PIN"}</label>
                  <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowPin(v => !v)} className={`absolute top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors ${isRTL ? "left-3" : "right-3"}`}>
                ...
                <div className={`bg-primary/8 rounded-2xl p-3 border border-primary/20 text-xs text-primary/80 space-y-1 ${isRTL ? "text-right" : ""}`}>
                  <p className="font-bold text-primary">{ar ? "بيانات تجريبية:" : "Demo credentials:"}</p>
                  <p>A12345678 / 4567 — Ahmed Ali (Egyptian)</p>
                  <p>B98765432 / 7890 — Muhammad Rahman (Indonesian)</p>
                </div>
                ...
                <button
                  ...
                  className="w-full h-12 bg-primary text-white rounded-2xl font-bold text-sm flex items-center justify-center gap-2 hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all active:scale-[0.98]"
                >
                  {loading ? <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <><LogIn className="w-4 h-4 flex-shrink-0" />{ar ? "دخول" : "Enter"}</>}
                </button>
              </motion.form>
            )}
          </AnimatePresence>
